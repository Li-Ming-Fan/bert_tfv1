...
class ModelForTrain(object):
    """ dataset = dataset_creator(settings)
        outputs = model_creator(settings, batch_data_tensors)
        optimizer = optimizer_creator(loss_tensor, settings)
        train_op_dict = train_op_dict_creator(loss_tensor, optimizer, settings)
    """
    def __init__(self, settings):
        ...
        if "logger" not in settings:
            self.logger = create_logger(settings["log_tag"], settings["log_path"])
        else:
    ...
    #
    def create_model_graph_and_sess(self, dataset_creator, model_creator, settings):
        ...
        with self._graph.as_default():
            ...
            batch_data_tensors = data_iter.get_next()
            #
            outputs = model_creator(settings, batch_data_tensors)
            #
        #
        ...
        #
    #
    def create_train_op_dict(self, optimizer_creator, train_op_dict_creator,
                             loss_tensor, settings):
        """ returning: train_op_dict
        """
        with self._graph.as_default():
            optimizer = optimizer_creator(loss_tensor, settings)
            train_op_dict = train_op_dict_creator(loss_tensor, optimizer, settings)
    ...
    #
    def train(self, train_op_dict, settings):
        """ train_op = train_op_dict["train_op"]
        """
        if "accum_ops" in train_op_dict:
            assert "grad_accum_steps" in settings, "grad_accum_steps not in settings"
            grad_accum_steps = settings["grad_accum_steps"]
            #
            accum_ops = train_op_dict["accum_ops"]
            zero_ops = train_op_dict["zero_ops"]
            #
            train_op = train_op_dict["train_op"]
            #
            train_op_dict_filtered = {}
            for key, value in train_op_dict.items():
                if key not in ["accum_ops", "zero_ops"]:
                    train_op_dict_filtered[key] = value
            #
        else:
            accum_ops = None
        #
        ...
        with self._graph.as_default():
            ...
            if accum_ops:
                for step in range(max_train_steps):
                    ...
                    #
                    outputs = sess.run(train_op_dict_filtered)
                ...
                for step in range(max_train_steps):
                    ...
                    outputs = sess.run(train_op_dict)
                    loss = outputs["loss_train"]
                    #
            #
            self._saver.save(self._sess, os.path.join(model_dir, model_name),
        ...
        #
    #
    def debug(self, train_op_dict, settings):
        """ train_op = train_op_dict["train_op"]
        """
        if "accum_ops" in train_op_dict:
            assert "grad_accum_steps" in settings, "grad_accum_steps not in settings"
...
def remove_from_trainable_variables(non_trainable_names, trainable_vars=None):
    ...
    #
#
def train_op_dict_creator_simple(loss_tensor, optimizer, settings):
    """
    """
    ...
    #
#
def train_op_dict_creator_multibatch(loss_tensor, optimizer, settings):
    """
    """
    tr_vars = tf.trainable_variables()
    global_step = tf.train.get_or_create_global_step()
    ...
    #
    (grads, _) = tf.clip_by_global_norm(accum_vars, clip_norm=1.0)
    #
    train_op = optimizer.apply_gradients(
        zip(grads, tr_vars), global_step=global_step)
    #
